...
async function uploadBackingTracksViaAPI() {
  try {
    ...
    for (let i = 0; i < trackMappings.length; i++) {
      ...
      try {
        ...
        // Optional fields that are allowed in the schema
...
if (import.meta.url === `file://${process.argv[1]}`) {
  log('🚀 Backing Track Upload via API Script Starting...', 'magenta');
  log('📁 Looking for audio files in test-assets directory...', 'cyan');
  
  uploadBackingTracksViaAPI().catch(error => {
...
export { uploadBackingTracksViaAPI };
