...
router.get('/order/:type/:id', authMiddleware, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Internal server error' });
      ...
      artistAvatar: purchase.track?.user?.avatar,
      price: purchase.price,
      purchasedAt: purchase.purchasedAt,
      paymentIntentId: purchase.paymentIntentId,
      refunded: purchase.refunded,
      trackId: purchase.track?._id,
      downloadUrl: purchase.track?._id ? `/tracks/${purchase.track._id}/download` : null
    }));

    // Format commission orders (as customer)
    const customerCommissionOrders = customerCommissions.map(commission => ({
      id: commission._id,
      type: 'commission_order',
      role: 'customer',
      status: commission.status,
      title: `Commission from ${commission.artist?.username || 'Unknown Artist'}`,
      artist: commission.artist?.username || 'Unknown Artist',
      artistAvatar: commission.artist?.avatar,
      price: commission.price,
      requirements: commission.requirements,
      createdAt: commission.createdAt,
      updatedAt: commission.updatedAt,
      expiryDate: commission.expiryDate,
    ...
    // Format commission orders (as artist)
    const artistCommissionOrders = artistCommissions.map(commission => ({
      id: commission._id,
      type: 'commission_order',
      role: 'artist',
      status: commission.status,
      title: `Commission for ${commission.customer?.username || 'Unknown Customer'}`,
      customer: commission.customer?.username || 'Unknown Customer',
    ...
    const allOrders = [...trackOrders, ...customerCommissionOrders, ...artistCommissionOrders]
...
router.get('/order/:orderId', authMiddleware, async (req, res) => {
  try {
    ...
    const { type } = req.query; // 'track_purchase' or 'commission_order'
    ...
    if (type === 'track_purchase') {
      ...
      const purchase = user?.purchasedTracks.id(orderId);
      if (!purchase) {
        return res.status(404).json({ error: 'Track purchase not found' });
      }
      ...
      if (purchase.paymentIntentId && purchase.paymentIntentId !== 'free') {
        try {
          const paymentIntent = await stripeClient.paymentIntents.retrieve(purchase.paymentIntentId);
          stripeDetails = {
      ...
      const orderDetails = {
        ...
        title: purchase.track?.title || 'Track Not Found',
        artist: purchase.track?.user?.username || 'Unknown Artist',
        artistEmail: purchase.track?.user?.email,
        artistAvatar: purchase.track?.user?.avatar,
        price: purchase.price,
        purchasedAt: purchase.purchasedAt,
        paymentIntentId: purchase.paymentIntentId,
    ...
    } else if (type === 'commission_order') {
      const commission = await CommissionRequest.findById(orderId)
        .populate('artist', 'username avatar email')
        .populate('customer', 'username email');
      ...
      // Check if user is involved in this commission
      if (commission.customer._id.toString() !== req.userId && commission.artist._id.toString() !== req.userId) {
        return res.status(403).json({ error: 'Not authorized to view this commission' });
      }
      ...
      const orderDetails = {
        id: commission._id,
        type: 'commission_order',
        role: commission.customer._id.toString() === req.userId ? 'customer' : 'artist',
        status: commission.status,
        artist: commission.artist?.username || 'Unknown Artist',
        artistEmail: commission.artist?.email,
        artistAvatar: commission.artist?.avatar,
        customer: commission.customer?.username || 'Unknown Customer',
        customerEmail: commission.customer?.email,
        price: commission.price,
        ...
        finishedTrackUrl: commission.finishedTrackUrl,
        previewTrackUrl: commission.previewTrackUrl,
        guideTrackUrl: commission.guideTrackUrl,
        cancellationReason: commission.cancellationReason,
        stripeDetails
      };
...
// Re-download track (for track purchases)
router.post('/redownload/:trackId', authMiddleware, async (req, res) => {
  try {
    const { trackId } = req.params;
    
    const user = await User.findById(req.userId);
    const hasPurchased = user.purchasedTracks.some(
      p => p.track.toString() === trackId && !p.refunded
    );

    if (!hasPurchased) {
      return res.status(403).json({ error: 'Track not purchased or has been refunded' });
    }

    // Redirect to the download endpoint
    res.json({ downloadUrl: `/tracks/${trackId}/download` });

  } catch (error) {
    console.error('Error processing redownload:', error);
    res.status(500).json({ error: 'Failed to process redownload' });  }
});
